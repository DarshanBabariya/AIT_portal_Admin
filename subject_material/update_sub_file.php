<?php 
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["sem"] != "" && $_REQUEST["subcode"] != "" && $_REQUEST["action"] != "" && $_REQUEST["fileid"] != "") {
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($_FILES["customFile"]["tmp_name"], $target_file)) {
                ...
                $_SESSION["success"] = "The $action file <b>". htmlspecialchars( basename( $_FILES["customFile"]["name"])). "</b> has been updated successfully.!!";
                header("Location: subject_material.php?sem=$sem&subcode=$subcode");
